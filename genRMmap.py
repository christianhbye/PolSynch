    ...
    if corr:
        ...
        for rmi in range(nRM):
            for rmj in range(nRM):
                for l in range(lmax):
                    if l < 200:
                        rho[rmi, rmj, l] = (rho_info[0] * l + rho_info[1])
                    else:
                        rho[rmi, rmj, l] = (rho_info[2] * l ** rho_info[3])
    return rho


# function to create covariance matrix to use in RM map generation.
# If rho is zero no correlations are added.
    ...
    for i in range(nRM):
        for j in range(i, nRM):
            if j == i:
                ...
                # rho is under squared root for Q,U to obtain rho in P
                cov[i, j] = 0.75 * np.sqrt(rho[i, j, ell]) * np.sqrt(ell ** alpha[i] * ell ** alpha[j])
                cov[j, i] = cov[i, j]
    return cov
...
def genRMcube_corr(nRM, nside, seed, cov, sigma):
    ...
    count = 0
    lmax = 3 * nside
    # Use integer division (//) to get array indices (Python 3 / returns float)
    ...
    for l in range(lmax):
        if l <= 2:
            alm_Q_wo[count:count + l + 1, :] = alm_U_wo[count:count + l + 1, :] = 0
        else:
            alm_Q_wo[count:count + l + 1, :] = 1. / np.sqrt(2) * (
                np.random.multivariate_normal(np.zeros(nRM), cov[l, :], l + 1)
                + 1j * np.random.multivariate_normal(np.zeros(nRM), cov[l, :], l + 1)
            )
            alm_U_wo[count:count + l + 1, :] = 1. / np.sqrt(2) * (
                np.random.multivariate_normal(np.zeros(nRM), cov[l, :], l + 1)
                + 1j * np.random.multivariate_normal(np.zeros(nRM), cov[l, :], l + 1)
            )
        count += l + 1
